...
#             return [permissions.AllowAny()]  # Разрешаем всем регистрироваться
#         elif self.action in ['activate_deactivate']:
#          return [permissions.IsAdminUser()]
#         return [permissions.IsAuthenticated()]


#     # Custom action to activate or deactivate a user
#     @action(detail=True, methods=['patch'], url_path='activate-deactivate', permission_classes=[IsAdminUser])
#     def activate_deactivate(self, request, pk=None):
#         user = self.get_object()
#         user.is_active = not user.is_active
#         user.save()
#         return Response({'status': 'User activation status updated'}, status=status.HTTP_200_OK)
